...
import com.example.meteorologia.databinding.FragmentVientoBinding
...
class VientoFragment : Fragment() {

    private lateinit var binding: FragmentVientoBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentVientoBinding.inflate(layoutInflater)
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                if (isAdded) {
                    if (snapshot.exists()) {
                        ...
                        val vientoList = productosList.map { Producto(it.id, it.viento) }

                        // Inicializar el adaptador solo con la temperatura
                        adapterProducto = AdapterProducto(vientoList, requireContext())
